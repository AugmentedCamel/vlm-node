...
def run_inference(vlm_prompt, prompt, image_paths):
    ...
    for image_path in image_paths:
        ...
        responses.append((parse_image_id(image_path), parse_image_timestamp(image_path), res.response.strip()))
        logger.info("Inference output: " + str(res))

    logger.info("Inference completed")

    # If multiple images were processed, join responses; otherwise return the single response
    if len(responses) == 1:
        _, _, single_response = responses[0]
        return {"response": single_response}
    else:
        # Join with newlines, preserving simple readability
        joined = "\n".join(r for _, _, r in responses)
